...
基於Qwen模型的露西亞角色AI對話系統，支持多種交互方式：終端對話、Discord機器人、圖形界面，以及語音識別功能。
先稍微說一下 以下文件是AI產生的 有可能有錯誤 所以實際使用還是要有點程式經驗 使用時以實際狀況為準
基本上是之前rushia ai的重構升級版 打掉全部使用提示詞和RAG建置的聊天機器人 相對於之前會胡言亂語 這個版本就不會了 可以多輪會談
目前還是學生蠻閒的 有問題或是建議我都可以快速回覆 也可以加我disord或IG或FB直接問我
對於跟真人潤羽露西亞還是有很大的差距 之後還是會更新新增更多特性 期待哪天變AI vtuber(X)
